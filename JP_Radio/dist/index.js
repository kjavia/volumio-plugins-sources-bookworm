...
const radio_controller_1 = __importDefault(require("./lib/controllers/radio-controller"));
const auth_1 = require("./lib/logic/auth");
const message_catalog_1 = require("./lib/utils/message-catalog");
const plugin_paths_1 = require("./lib/utils/plugin-paths");
const area_name_1 = require("./lib/consts/area-name");
const logger_1 = require("./lib/utils/logger");
const radio_time_1 = require("./lib/utils/radio-time");
/**
...
class ControllerJpRadio {
    ...
    constructor(context) {
        ...
        this.logger = new logger_1.LoggerEx(context.logger);
    ...
    async restartPlugin() {
        ...
        try {
            ...
            this.commandRouter.pushToastMessage('error', message_catalog_1.messageCatalog.get('RESTART_FAILED_TITLE'), message_catalog_1.messageCatalog.get('RESTART_FAILED_MESSAGE'));
        }
    }
    /**
    ...
    showRestartModal() {
        const message = {
            title: message_catalog_1.messageCatalog.get('RESTART_MODAL_TITLE'),
            message: message_catalog_1.messageCatalog.get('RESTART_MODAL_MESSAGE'),
            size: 'lg',
            buttons: [
                {
     ...
     */
    pushSettingsSavedToast() {
        this.commandRouter.pushToastMessage('success', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('SETTINGS_SAVED'));
    }
    /**
     ...
     */
    populateSelectValue(content, currentValue) {
     ...
     */
    async populateRadikoAreasSection(section) {
        ...
        for (const region of area_name_1.AREA_REGIONS) {
            section.content.push({ label: region.name });
            for (const areaId of region.areaIdArray) {
                let label = area_name_1.AREA_KANJI.get(areaId);
                if (label === undefined) {
                    label = areaId;
                }
                if (areaId === myAreaId) {
                    label += message_catalog_1.messageCatalog.get('RADIKO_MY_AREA');
                }
    ...
    async saveNetworkSetting(data) {
        ...
        if (portChanged === true) {
            ...
            (0, radio_time_1.setRadioDelay)(newDelay);
    ...
    async saveRadikoAccount(data) {
        ...
        const updated = Object.keys(data).some((key) => this.config.get(key) !== data[key]);
        if (updated === true) {
     ...
     */
    async saveBrowseModeSetting(data) {
     ...
     * @param data 保存ボタンから渡される選択値。
     */
    async saveAlbumartSetting(data) {
     ...
     */
    async clearStationLogoCache() {
        this.logger.info('IDX_I021');
        try {
            const files = await fs_1.default.promises.readdir(plugin_paths_1.ASSETS_IMAGES_DIR).catch(() => []);
            const logoFiles = files.filter((file) => file.endsWith('_logo.png'));
            await Promise.all(logoFiles.map((file) => fs_1.default.promises.unlink(path_1.default.join(plugin_paths_1.ASSETS_IMAGES_DIR, file))));
            this.commandRouter.pushToastMessage('success', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('STATION_LOGO_CLEAR'));
            this.showRestartModal();
        }
        catch (error) {
            this.logger.error('IDX_E012', error);
            this.commandRouter.pushToastMessage('error', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('ERROR_GENERIC'));
        }
    }
    /**
    ...
    async saveTimetableDisplaySetting(data) {
        ...
        if (updated === true) {
            this.appRadio?.updateTimetableDisplay(this.getConfigNumber('programPeriodFrom', 7), this.getConfigNumber('programPeriodTo', 0), this.config.get('timeFormat') || 'yyyy/MM/dd HH:mm-HH:mm');
            this.pushSettingsSavedToast();
        }
    }
    /**
     ...
     */
    async saveRadikoAreasSetting(data) {
    ...
    onVolumioStart() {
        ...
        try {
            ...
            message_catalog_1.messageCatalog.setLanguage(langCode);
            this.commandRouter.sharedVars.registerCallback('language_code', (newLangCode) => {
                message_catalog_1.messageCatalog.setLanguage(newLangCode || 'en');
            });
    ...
    onVolumioShutdown() {
        ...
        const defer = kew_1.default.defer();
        this.onStop().then(() => defer.resolve(), (error) => defer.reject(error));
        return defer.promise;
    }
    /**
    ...
    onStart() {
        ...
        try {
            ...
            const account = (0, auth_1.createLoginAccount)(radikoUser, radikoPass);
            (0, radio_time_1.setRadioDelay)(networkDelay);
            this.appRadio = new radio_controller_1.default(servicePort, this.logger, account, this.commandRouter, this.serviceName, browseMode1, browseMode2, radikoAreaIdArray, tempo, programPeriodFrom, programPeriodTo, timeFormat, albumartType);
        }
        catch (error) {
            this.logger.error('IDX_E002', error);
            this.commandRouter.pushToastMessage('error', message_catalog_1.messageCatalog.get('ERROR_BOOT_TITLE'), error?.message || message_catalog_1.messageCatalog.get('ERROR_UNKNOWN'));
            defer.reject(error);
            return defer.promise;
        }
        this.appRadio.start()
            ...
            if (error.code === 'EADDRINUSE') {
                const message = message_catalog_1.messageCatalog.get('ERROR_PORT_IN_USE', this.config.get('servicePort'));
                this.logger.error('IDX_E003', message);
                this.commandRouter.pushToastMessage('error', message_catalog_1.messageCatalog.get('ERROR_BOOT_TITLE'), message);
            }
            else {
                this.commandRouter.pushToastMessage('error', message_catalog_1.messageCatalog.get('ERROR_BOOT_TITLE'), error.message || message_catalog_1.messageCatalog.get('ERROR_UNKNOWN'));
            }
    ...
    removeOwnQueueItems() {
        ...
        const filteredQueue = arrayQueue.filter((item) => item.service !== this.serviceName);
    ...
    getUIConfig() {
        ...
        const langCode = this.commandRouter.sharedVars.get('language_code') || 'en';
        this.commandRouter.i18nJson(path_1.default.join(plugin_paths_1.I18N_DIR, `strings_${langCode}.json`), path_1.default.join(plugin_paths_1.I18N_DIR, 'strings_en.json'), plugin_paths_1.UI_CONFIG_PATH)
            .then(async (uiconf) => {
            const servicePort = this.config.get('servicePort');
            const networkDelay = this.getConfigNumber('networkDelay', 20);
            const radikoUser = this.config.get('radikoUser');
     ...
     */
    showProgInfoModal(data) {
        let message = `<div>${data.artist}</div>`;
        if (data.album !== '') {
            message += `<div>${message_catalog_1.messageCatalog.get('PROGINFO_PERFORMER')}${data.album}</div>`;
        }
        const modalMessage = {
            title: message_catalog_1.messageCatalog.get('PROGINFO_PROG_INFO') + data.title,
            message,
            size: 'lg',
            buttons: [
                {
                    name: message_catalog_1.messageCatalog.get('PROGINFO_PLAY'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
                ...
                {
                    name: message_catalog_1.messageCatalog.get('PROGINFO_ADD_TO_QUEUE'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
                ...
                {
                    name: message_catalog_1.messageCatalog.get('PROGINFO_ADD_TO_FAVOURITES'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
        ...
        if (queryStr !== undefined) {
            ...
            if (ft !== null && to !== null) {
                ...
                const farFutureCutoffDate = (0, radio_time_1.addDaysToDateOnly)((0, radio_time_1.parseRadioTime)(currentRadioTime).date, 7);
                if ((0, radio_time_1.parseRadioTime)(ft).date > farFutureCutoffDate) {
                    // 7日以上先の番組は再生/キュー/お気に入りボタンを全て非表示
                    modalMessage.buttons.splice(0, 3);
                }
                else if ((0, radio_time_1.getProgramTimeStatus)(ft, to, currentRadioTime) !== 'past') {
    ...
    showProgRegModal(data) {
        ...
        const modalMessage = {
            ...
            buttons: [
                {
                    name: message_catalog_1.messageCatalog.get('PROGREG_NEXT_DAY'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
                ...
                {
                    name: message_catalog_1.messageCatalog.get('PROGREG_NEXT_WEEK'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
                ...
                {
                    name: message_catalog_1.messageCatalog.get('PROGREG_NEXT_2WEEK'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
                ...
                {
                    name: message_catalog_1.messageCatalog.get('PROGREG_UPDATE_FAVOURITES'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
                ...
                {
                    name: message_catalog_1.messageCatalog.get('PROGREG_REMOVE_FROM_FAVOURITES'),
                    class: 'btn btn-info',
                    emit: 'callMethod',
                    payload: {
                        endpoint: `music_service/${this.serviceName}`,
     ...
     */
    async changeDateFromProgRegModal(data) {
        ...
        const newFt = (0, radio_time_1.addDaysToRadioTime)(ft, data.days);
        const newTo = (0, radio_time_1.addDaysToRadioTime)(to, data.days);
        const appRadio = this.appRadio;
        if (appRadio === null) {
            return;
        }
     ...
     */
    async updateFavouriteFromProgRegModal(data) {
        this.logger.info('IDX_I023', data.uri);
        await this.commandRouter.playListManager.commonRemoveFromPlaylist(this.commandRouter.playListManager.favouritesPlaylistFolder, 'radio-favourites', 'webradio', data.oldUri);
        await this.commandRouter.playListManager.commonAddToPlaylist(this.commandRouter.playListManager.favouritesPlaylistFolder, 'radio-favourites', 'webradio', data.uri, data.title, data.albumart);
        this.commandRouter.pushToastMessage('success', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('FAVOURITE_UPDATED', data.title));
    }
    /**
     * {@link showProgRegModal}の「お気に入りから削除」ボタンから呼ばれる。
     * @param data `oldUri`(削除対象の登録URI)を含む番組情報。
     */
    async removeFavouriteFromProgRegModal(data) {
        this.logger.info('IDX_I024', data.oldUri);
        await this.commandRouter.playListManager.commonRemoveFromPlaylist(this.commandRouter.playListManager.favouritesPlaylistFolder, 'radio-favourites', 'webradio', data.oldUri);
        this.commandRouter.pushToastMessage('success', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('FAVOURITE_REMOVED', data.title));
    }
    /**
     * 番組情報モーダルの「再生」ボタンから呼ばれる。対象トラックを再生キューの先頭に追加して即再生する。
     * @param data {@link showProgInfoModal}のボタンから渡されるトラック情報。
     */
    playFromProgInfoModal(data) {
     ...
     * @param data {@link showProgInfoModal}のボタンから渡されるトラック情報。
     */
    addQueueFromProgInfoModal(data) {
        ...
        if (uri.includes('/radiko/play/') === true) {
            ...
            if (queryStr !== undefined && queryStr !== '') {
                ...
                if (ft === null || to === null || ft.length !== 14 || to.length !== 14) {
                    ...
                    this.commandRouter.pushToastMessage('error', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('ERROR_INVALID_TIMEFREE_PARAMS'));
                    uri = liveUri;
                }
                else {
                    const status = (0, radio_time_1.getProgramTimeStatus)(ft, to, (0, radio_time_1.getCurrentRadioTime)());
                    if (status === 'future') {
                        this.logger.warn('IDX_W002', uri);
                        this.commandRouter.pushToastMessage('info', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('WARNING_SWITCH_LIVE_FUTURE'));
                        uri = liveUri;
                    }
                    else if (status === 'live') {
                        this.logger.warn('IDX_W003', uri);
                        this.commandRouter.pushToastMessage('info', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('WARNING_SWITCH_LIVE_CATCHUP'));
                        uri = liveUri;
                    }
                }
            }
    ...
    getTrackInfo(uri) {
        ...
        kew_1.default.resolve()
            ...
            .then((track) => defer.resolve([track]))
            .fail((error) => {
    ...
    goto(data) {
        ...
        let dateOnly = (0, radio_time_1.getCurrentRadioDate)();
        if (queryStr !== undefined) {
            const ft = new URLSearchParams(queryStr).get('ft');
            if (ft !== null) {
                dateOnly = (0, radio_time_1.parseRadioTime)(ft).date;
            }
        }
        kew_1.default.resolve()
     ...
     */
    buildFavouriteTitle(title, artist) {
     ...
     */
    addFavouriteFromProgInfoModal(data) {
        this.logger.info('IDX_I018', data.uri);
        this.commandRouter.pushToastMessage('success', message_catalog_1.messageCatalog.get('APP_TITLE'), message_catalog_1.messageCatalog.get('FAVOURITE_ADDED', data.title));
        this.commandRouter.playListManager.commonAddToPlaylist(this.commandRouter.playListManager.favouritesPlaylistFolder, 'radio-favourites', 'webradio', data.uri, this.buildFavouriteTitle(data.title, data.artist), data.albumart);
    }
    /**
    ...
    addToFavourites(data) {
        ...
        return this.commandRouter.playListManager.commonAddToPlaylist(this.commandRouter.playListManager.favouritesPlaylistFolder, 'radio-favourites', 'webradio', data.uri, this.buildFavouriteTitle(data.title ?? '', data.artist), data.albumart);
    }
    /**
    ...
    removeFromFavourites(data) {
        ...
        return this.commandRouter.playListManager.commonRemoveFromPlaylist(this.commandRouter.playListManager.favouritesPlaylistFolder, 'radio-favourites', 'webradio', data.uri);
    }
}
